const db = require("../../Models/Payroll/PayrollDeductionModel");
...
const postData = async (req, res) => {
  try {
    ...
    const PayrollData = await db.create({
      ...
      percentage_of_basic,
      limit_per_month,
      fixed_amount: deduction_type === "Percentage" ? undefined : fixed_amount,
    });
...
const deleteData = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Payroll Deduction Removed successfully",
      });
  } catch (error) {
    res.status(404).json({ message: error.message });
  }
